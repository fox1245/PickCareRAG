...
# 나머지 도구들 (PDFask, HWPask 등)은 기존 그대로 유지. 필요 시 비슷하게 웹 fallback 추가 가능.
@mcp.tool()
...
async def rag_query(request: QueryRequest = Body(...)):
    try:
        ...
        # 신규: 쿼리에 웹 검색 키워드 있으면 agent로 fallback
        ...
        if request.tool == "SharedFolderSearchAndRAG":
            ...
            return QueryResponse(  # 수정: return 추가, 이전에 없었을 수 있음
                result=result,
                source="System Date",
                timestamp=datetime.now().isoformat()
            )
        ...
        # ... (다른 도구들 추가: GetCurrentDate, prompt_maker 등 필요 시 if-elif 확장)
            ...
            tools = tools_list
            # 추가: system prompt로 도구 사용 지시 (날짜 + 웹 검색 트리거 강화)
        ...
        logger.error(f"Error in rag_query: {e}", exc_info= True) # 수정: exc_info=True로 스택 추가
