...
async def rag_query(request: QueryRequest = Body(...)):
    try:
        ...
        async def run_agent():
            ...
            return agent_response["messages"][-1].content  # result 반환
        
        # web_search_needed를 최상단에서 체크: 필요 시 바로 agent 호출 (fallback 일관성)
        if web_search_needed:
            logger.info("웹 검색 필요 - agent로 fallback")
            result = await run_agent()
            return QueryResponse(
                result=result,
                source=request.file_path or "Shared Folder (with Agent Fallback)",
