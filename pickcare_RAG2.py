...
from langchain_core.messages import HumanMessage, SystemMessage  # langgraph를 위한 import 추가 (SystemMessage 신규 추가)
...
async def rag_query(request: QueryRequest = Body(...)):
    try:
        ...
        # ... (다른 도구들 추가: GetCurrentDate, prompt_maker 등 필요 시 if-elif 확장)
            ...
            # 추가: system prompt로 도구 사용 지시 (날짜 등 특정 쿼리 트리거)
            system_message = SystemMessage(content="너는 도움이 되는 AI야. 날짜 관련 질문(오늘 날짜, 현재 날짜 등)이 나오면 반드시 GetCurrentDate 도구를 호출해서 정확한 시스템 날짜를 가져와. 다른 도구도 필요 시 사용하고, 이전 대화 히스토리를 항상 참고해.")
            agent = create_react_agent(
                model="openai:gpt-5-mini",  # 모델 변경: gpt-5-mini로 업그레이드
                tools=tools,
                messages_modifier=system_message  # 추가: system prompt로 agent 지시
            )
